    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Add_to_cart.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                userRef.document(Objects.requireNonNull(userAuth.getUid())).collection("products")
                        .document(item.getId() + "")
                        .set(item).addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(Void aVoid) {
                        Toast.makeText(ItemDetailsActivity.this, "The item has been successfully saved", Toast.LENGTH_SHORT).show();
                    }
                }).addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception e) {
                        Toast.makeText(ItemDetailsActivity.this, "Something went wrong!ITEM NOT SAVED", Toast.LENGTH_SHORT).show();
                    }
                });
                ;
